...
def  main():
  ...
  installer = "\installer\VaiaOneBitPlus Installer.exe"
   
  if demo:
    installer = "\installer\VaiaOneBitPlus Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\VaiaOneBitPlus manual.pdf" 
  ]
  ...
  ZIPNAME = "VaiaOneBitPlus-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "VaiaOneBitPlus-v" + FULLVERSIONSTR + "-win-demo.zip"
